...
exports.apiLogin = async function (req, res) {
  ...
  if (result) {
    let data = {
      token: jwt.sign({ _id: student.data._id, name: student.data.name, email: student.data.email }, process.env.JWTSECRET, { expiresIn: tokenLasts }),
      id: student.data._id,
...
exports.apiRegister = async function (req, res) {
  ...
  if (result) {
    let data = {
      token: jwt.sign({ _id: student.data._id, name: student.data.fName, email: student.data.email }, process.env.JWTSECRET, { expiresIn: tokenLasts }),
      id: student.data._id,
      name: student.data.name,
      role: "student"
    }
