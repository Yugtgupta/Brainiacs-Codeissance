  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      ...
      try {
        ...
        console.log(user.id)
        const response = await Axios.get(`/get-my-chat/${user.id}/${mentorId}`, {
          headers: {
    ...
    <div className="max-w-screen-md mx-auto bg-gray-100 h-screen flex flex-col">
      ...
      <div className="bg-blue-600 text-white p-4 rounded-t-lg flex items-center justify-between shadow-md">
        <div className="flex items-center">
          <img src="https://randomuser.me/api/portraits/men/32.jpg" alt="Mentor Avatar" className="w-10 h-10 rounded-full mr-3" />
          <div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map(message => (
          <div key={message.id} className={`flex ${message.senderName === user?.role ? "justify-end" : "justify-start"}`}>
            <div className={`p-3 rounded-lg max-w-xs shadow-md ${message.senderName === user?.role ? "bg-blue-500 text-white" : "bg-gray-200 text-black"}`}>
      ...
      <div className="p-4 bg-white border-t flex items-center space-x-4">
        <input type="text" className="flex-1 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Type your message..." value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyPress={e => e.key === "Enter" && handleSendMessage()} />
        <button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition" onClick={handleSendMessage}>
          Send
        </button>
      </div>
    </div>
