...
const ScholarshipList = () => {
  ...
  const filteredScholarships = scholarships.filter(scholarship => scholarship.name.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      ...
      // Split the string using regex that matches the pattern of a number followed by a period and a space
      const recommendationsArray = pyData.recommendations.split(/\d+\.\s+/).filter(Boolean)
      ...
      setRecommendedScholarships(recommendationsArray)
    } catch (error) {
  ...
  useEffect(() => {
    console.log("Scholarships", recommendedScholarships)
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 bg-gray-100 shadow-lg rounded-lg mt-10">
      ...
      <div className="flex mb-4">
        <input type="text" placeholder="Search Scholarships..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="border border-gray-300 rounded-l-md p-3 flex-grow focus:outline-none focus:ring-2 focus:ring-indigo-500" />
      ...
      <button onClick={handleGetRecommendations} className="mb-6 w-full bg-indigo-600 text-white font-bold py-3 rounded-md hover:bg-indigo-700 transition duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {!fetchedData &&
          filteredScholarships.map((scholarship, index) => (
            <div key={index} className="p-6 border border-gray-200 rounded-lg shadow-md hover:shadow-xl transition duration-300 bg-white" style={{ height: maxCardHeight }}>
      ...
      {fetchedData && recommendedScholarships.length > 0 && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-10 w-full">
            {recommendedScholarships.map((scholarship, index) => (
              <div key={index} className="p-6 border border-gray-200 rounded-lg shadow-md hover:shadow-xl transition duration-300 bg-white w-full" style={{}}>
                <h3 className="text-xl font-bold text-indigo-600 mb-2">{scholarship}</h3>
                {/* <p className="mt-2 text-gray-700 overflow-hidden" style={{ maxHeight: "100px", overflowY: "hidden" }}>
                ...
                {/* {scholarship?.eligibility && scholarship?.eligibility.length > 100 && (
                  <a
                    href="#"
                    className="text-indigo-500 hover:underline mt-2 inline-block"
                    onClick={e => {
